...
def train():
  ...
  mlp = MLP(3* 32* 32, dnn_hidden_units, 10).cuda();
  print(mlp);
  mlp.apply(mlp.init_weights);
  ...
  x_test = torch.autograd.Variable(torch.from_numpy(x_test)).cuda();
  ...
  while (1):
      ...
      if ((steps % eval_freq) == 0):
          ...
          print('---accuracy: ', rate, '---', loss.data[0]);
          tmp_accuracy_rates += [rate]
          tmp_loss_lists += [loss.data[0]];
